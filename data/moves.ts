...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	airslash: {
		...
		pp: 15,
		priority: 0,
		flags: { protect: 1, mirror: 1, distance: 1, metronome: 1, slicing: 1, wind: 1 },
		secondary: {
			chance: 30,
	...
	auroraveil: {
		...
		pp: 20,
		priority: 0,
		flags: { snatch: 1, metronome: 1, wondergate: 1 },
	...
	beakblast: {
		...
		pp: 15,
		priority: -3,
		flags: { protect: 1, failmefirst: 1, nosleeptalk: 1, failcopycat: 1, failinstruct: 1, bullet: 1, pulse: 1 },
	...
	boneclub: {
		...
		onTryHit(pokemon) {
			...
			if (pokemon.side.removeSideCondition('reflect')) {
				return true;
			}
			if (pokemon.side.removeSideCondition('lightscreen')) {
				return true;
			}
			if (pokemon.side.removeSideCondition('auroraveil')) {
				return true;
			}
		},
		secondary: {
	...
	darkpulse: {
		...
		pp: 15,
		priority: 0,
		flags: { protect: 1, mirror: 1, distance: 1, metronome: 1, pulse: 1, bullet: 1 },
		secondary: {
			chance: 20,
			volatileStatus: 'flinch',
		},
	...
	dragonrush: {
		...
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1 },
		onModifyMove(move, pokemon, target) {
			if (this.field.getPseudoWeather('tailwind')) {
				move.accuracy = true;
			}
		},
		secondary: {
	...
	electricterrain: {
		...
		pp: 10,
		priority: 0,
		flags: { nonsky: 1, metronome: 1, wondergate: 1 },
	...
	electroball: {
		...
		basePowerCallback(pokemon, target, move) {
			if (target.newlySwitched || this.queue.willMove(target)) {
				this.debug('Electro Ball damage boost');
				return move.basePower * 1.5;
			}
			this.debug('Electo Ball NOT boosted');
			return move.basePower;
		},
		category: "Special",
		name: "Electro Ball",
	...
	expandingforce: {
		...
		onBasePower(basePower, source) {
			if (this.field.isTerrain('psychicterrain')) {
				if (source.isGrounded() || source.ability === 'lakeguardian') {
	...
	flashcannon: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, metronome: 1, bullet: 1, pulse: 1, light: 1 },
		secondary: {
			chance: 20,
			boosts: {
				spd: -1,
	...
	gyroball: {
		...
		pp: 5,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, bullet: 1, pulse: 1 },
		secondary: null,
		target: "normal",
		type: "Steel",
		zMove: { basePower: 160 },
	...
	healpulse: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, reflectable: 1, distance: 1, heal: 1, allyanim: 1, metronome: 1, pulse: 1, bullet: 1 },
		onHit(target, source) {
			let success = false;
	...
	iceball: {
		...
		pp: 20,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, metronome: 1, failinstruct: 1, bullet: 1, pulse: 1, noparentalbond: 1 },
		onModifyMove(move, pokemon, target) {
	...
	lightscreen: {
		...
		pp: 30,
		priority: 0,
		flags: { snatch: 1, metronome: 1, wondergate: 1, light: 1 },
		sideCondition: 'lightscreen',
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
	...
	magicroom: {
		...
		pp: 10,
		priority: 0,
		flags: { mirror: 1, metronome: 1, wondergate: 1 },
	...
	obstruct: {
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				if (this.checkMoveMakesContact(move, source, target)) {
					this.boost({ spa: -2 }, source, target, this.dex.getActiveMove("Obstruct"));
				}
				return this.NOT_FAIL;
			},
			onHit(target, source, move) {
	...
	photongeyser: {
		...
		onPrepareHit(pokemon, target, move) {
			if (target.getStat('spd', false, true) > target.getStat('def', false, true)) move.category = 'Physical';
		},
		ignoreAbility: true,
		secondary: null,
		target: "normal",
	...
	pollenpuff: {
		...
		pp: 15,
		priority: 0,
		flags: { protect: 1, mirror: 1, allyanim: 1, metronome: 1, bullet: 1, pulse: 1 },
		onTryHit(target, source, move) {
	...
	psychicterrain: {
		...
		condition: {
			...
			onTryHit(target, source, effect) {
				...
				if (target.isSemiInvulnerable() || target.isAlly(source) || target.ability === 'lakeguardian') return;
				...
				if (target.ability === 'lakeguardian') return null;
				this.add('-activate', target, 'move: Psychic Terrain');
				return null;
			},
			onBasePowerPriority: 6,
			onBasePower(basePower, attacker, defender, move) {
				if (move.type === 'Psychic' && !attacker.isSemiInvulnerable()) {
					if (attacker.isGrounded() || attacker.ability === 'lakeguardian') {
	...
	raindance: {
		...
		pp: 5,
		priority: 0,
		flags: { metronome: 1, wondergate: 1 },
		weather: 'RainDance',
		secondary: null,
		target: "all",
		type: "Water",
	...
	roaroftime: {
		...
		pp: 5,
		priority: 0,
		flags: { recharge: 1, protect: 1, mirror: 1, metronome: 1, cantusetwice: 1, sound: 1 },
		secondary: null,
		target: "normal",
		type: "Dragon",
		contestType: "Beautiful",
	...
	sandstorm: {
		...
		pp: 10,
		priority: 0,
		flags: { metronome: 1, wind: 1, wondergate: 1 },
		weather: 'Sandstorm',
		secondary: null,
		target: "all",
		type: "Rock",
	...
	skyuppercut: {
		...
		onEffectiveness(typeMod, target, type, move) {
			...
			if (!target.runEffectiveness(move)) {
				if (target.hasType('Flying')) return 2;
			}
		},
	...
	smackdown: {
		...
		basePowerCallback(pokemon, target, move) {
			if (target.getVolatile('smackdown')) {
				this.debug('BP doubled from status condition');
				return move.basePower * 1.5;
			}
			return move.basePower;
	...
	spikes: {
		...
		pp: 20,
		priority: 0,
		flags: { reflectable: 1, nonsky: 1, metronome: 1, mustpressure: 1, wondergate: 1 },
	...
	springtidestorm: {
		...
		onModifyMove(move, pokemon, target) {
			if (target && ['sunnyday, desoleteland'].includes(target.effectiveWeather())) {
				move.accuracy = true;
			}
		},
		secondary: {
	...
	stealthrock: {
		...
		pp: 20,
		priority: 0,
		flags: { reflectable: 1, metronome: 1, mustpressure: 1, wondergate: 1 },
	...
	stickyweb: {
		...
		pp: 20,
		priority: 0,
		flags: { reflectable: 1, metronome: 1, wondergate: 1 },
	...
	doublestone: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, metronome: 1, noparentalbond: 1, pulse: 1, bullet: 1 },
		multihit: 2,
		smartTarget: true,
		secondary: null,
		target: "normal",
	...
	darkrays: {
		...
		pp: 20,
		priority: 0,
		flags: { protect: 1, mirror: 1, distance: 1, metronome: 1, bullet: 1, pulse: 1, light: 1 },
		secondary: null,
		target: "any",
		type: "Dark",
		contestType: "Beautiful",
	...
	runichand: {
		...
		basePower: 80,
		basePowerCallback(pokemon, target, move) {
			if (target.hasAbility('Wandering Spirit')) {
				this.debug('BP doubled from ability Disruption');
				return move.basePower * 2;
			}
			return move.basePower;
		},
	...
	cannonade: {
		...
		onBasePower(basePower, source, target, move) {
			if (target.runEffectiveness(move) > 0) {
				// Placeholder
				this.debug(`cannonade super effective buff`);
				return this.chainModify([5461, 4096]);
			}
		},
		multihit: [2, 5],
	...
	wildfire: {
		...
		onBasePower(basePower, source, target, move) {
			if (target.runEffectiveness(move) > 0) {
				// Placeholder
				this.debug(`wildfire super effective buff`);
				return this.chainModify([5461, 4096]);
			}
		},
		secondary: {
	...
	breathoflife: {
		...
		terrain: 'grassyterrain',
		onHit(target, source, move) {
			this.add('-activate', source, 'move: Breath of Life');
			let success = false;
			const allies = [...target.side.pokemon, ...target.side.allySide?.pokemon || []];
			for (const ally of allies) {
				if (ally !== source && !this.suppressingAbility(ally)) {
	...
	reflectray: {
		...
		pp: 15,
		priority: 0,
		flags: { protect: 1, mirror: 1, sound: 1, bypasssub: 1, metronome: 1, light: 1 },
		secondary: null,
		target: "normal",
		type: "Steel",
		contestType: "Beautiful",
	...
	citriccannon: {
		...
		pp: 15,
		priority: 0,
		flags: { protect: 1, mirror: 1, defrost: 1, metronome: 1, bullet: 1, pulse: 1 },
		thawsTarget: true,
		secondary: {
			chance: 30,
			status: 'brn',
	...
	galestrike: {
		...
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, metronome: 1, punch: 1, wind: 1 },
		secondary: null,
		target: "normal",
		type: "Flying",
		contestType: "Clever",
	...
	featherstorm: {
		...
		onAfterHit(source, target, move) {
			if (source.baseSpecies.name === 'Zapdos' || move.hasBounced) {
				if (this.randomChance(3, 10)) {
					target.trySetStatus('par', source, move);
				}
			}
			if (source.baseSpecies.name === 'Articuno' || move.hasBounced) {
				if (this.randomChance(3, 10)) {
					target.trySetStatus('frb', source, move);
				}
			}
			if (source.baseSpecies.name === 'Moltres' || move.hasBounced) {
				if (this.randomChance(3, 10)) {
					target.trySetStatus('brn', source, move);
				}
			}
	...
	sparkshow: {
		...
		flags: { bypasssub: 1, noassist: 1, failcopycat: 1 },
		onTry(source, target, move) {
			if (source.species.name === 'Plusle' || move.hasBounced) {
				return;
			}
			if (source.species.name === 'Minun' || move.hasBounced) {
				return;
			}
			this.add('-fail', source, 'move: Spark Show');
			this.hint("Only a Pokemon whose form is Plusle or Minun can use this move.");
			return null;
		},
		onAfterMove(target, source, move) {
			if (source.species.name === 'Plusle' || move.hasBounced) {
				this.boost({ atk: 1, spa: 1 });
				for (const allyActive of source.allies()) {
	...
	luckyspin: {
		...
		onAfterMove(pokemon) {
			...
			const selectedStats = stats.slice(0, 3);
